/* Memories Gallery Script */
    ...
    function cardTemplate(m){
      ...
      return `
        <article class="card" data-id="${m.id}" data-tags="${(m.tags||[]).join(',')}" data-title="${escapeHtml(m.title)}" data-caption="${escapeHtml(m.caption)}">
    ...
    saveMemory.addEventListener('click', async ()=>{
      ...
      if(!title || !caption){ alert('Add a title and caption, please 🙂'); return; }

      let src = '';
      if(file){
        // Store image as DataURL (LocalStorage); for real server, upload and store URL
        src = await toDataURL(file);
      } else {
        src = "assets/memories/placeholder.jpg"; // fallback
      }

      const id = 'm' + Math.random().toString(36).slice(2,8);
      const m = { id, src, title, date, tags:[tag], caption };
      memories.unshift(m);
      saveMemories(memories);
      renderGallery(currentTag, searchBox.value);
      // reset
      fileInput.value=''; titleInput.value=''; captionInput.value='';
      alert('Memory saved! 💖');
    });
